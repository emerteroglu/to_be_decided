...
const arbcontractaddr='INSERT CONTRACT ADRESS HERE'
const gas=web3.utils.toWei('225','gwei');
var contract = new web3.eth.Contract(abi(),arbcontractaddr);

while(1)
    for(var i=1;i<=91;i+=10){
     try{
        let amountin=web3.utils.toWei((i.toString(10)),'ether');
        let data=await contract.methods.findBestPathWithGas(amountin,WAVAX,WAVAX,4,gas).call();
        let diff=(data.amounts[data.amounts.length-1]-data.amounts[0]);
        diff=web3.utils.fromWei(diff.toString(10),'ether');

        if(diff>=0.2){console.log(i +' AVAX ARB opportunity '+ diff);}
}
    catch(err){
      continue;
};
